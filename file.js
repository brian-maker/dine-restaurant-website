...
const events = document.querySelectorAll("li.gathering-event")
const underlines = document.querySelectorAll("div.gathering-event-underline")
...
if(windowSize > breakPoints [0] && windowSize < breakPoints[1]){
    suffix =1
}else if(windowSize > breakPoints[2]){
    suffix=2
}
...
setInterval(()=>{
    ...
    if(slide ==(slides.length -1))slide=0;else slide++
},5000);

function generateImageComponent(slide,suffix){
    let newImage = document.createElement("img")
    newImage.setAttribute("class","gathering-slide-image-container-image")
